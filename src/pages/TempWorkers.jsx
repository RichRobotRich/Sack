...
export default function TempWorkers() {
  ...
  const [workerForm, setWorkerForm] = useState({ full_name: '', agency: '', phone: '', skills: '' });
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const nonEfProjectIds = projectsData.filter(p => !p.is_ef_project).map(p => p.id);
      const nonEfRows = rowsData.filter(row => nonEfProjectIds.includes(row.project_id));
      
      setTempWorkers(workersData);
      setTempAssignments(assignmentsData);
      setProjects(projectsData.filter(p => !p.is_ef_project));
      setProjectRows(nonEfRows);
    } catch (error) {
  ...
  // Generate 9 weeks starting from current week
  ...
  const generatePDF = () => {
    ...
    doc.text('Zusatzpersonal - Planung', margin, y);
    y += 8;
    ...
    doc.save(`Zusatzpersonal_${format(weeks[0].weekStart, 'yyyy-MM-dd')}.pdf`);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 no-print">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-1">
            Leiharbeiter verwalten und einplanen (9 Wochen)
          </p>
        </div>
      ...
      <div className="hidden print-only text-center mb-4">
        <h1 className="text-xl font-bold">Zusatzpersonal - Planung</h1>
      ...
      <Card className="border-0 shadow-sm no-print dark:bg-gray-800 dark:border-gray-700">
        <CardHeader>
          <CardTitle className="text-lg dark:text-white">Verfügbare Leiharbeiter</CardTitle>
        </CardHeader>
        <CardContent>
          {tempWorkers.length === 0 ? (
            <div className="text-center py-8 text-gray-500 dark:text-gray-400">
              <UserPlus className="w-10 h-10 mx-auto mb-2 opacity-50" />
              <p>Keine Leiharbeiter angelegt</p>
            </div>
          ) : (
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-gray-50 dark:bg-gray-900">
              <TableHead className="w-48 sticky left-0 bg-gray-50 dark:bg-gray-900 z-10 dark:text-gray-300">Baustelle</TableHead>
              {weeks.map((week, idx) => (
            ...
            {projectRows.length === 0 ? (
              <TableRow>
                <TableCell colSpan={11} className="text-center py-8 text-gray-500 dark:text-gray-400">
                  ...
                  <p>Keine Baustellen hinzugefügt</p>
                  <Button 
                    variant="outline" 
                    size="sm" 
                    className="mt-2"
      ...
      <Dialog open={addProjectDialogOpen} onOpenChange={setAddProjectDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Baustelle hinzufügen</DialogTitle>
          </DialogHeader>
          <div className="py-4">
            <select
      ...
      <Dialog open={weekDialogOpen} onOpenChange={setWeekDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>
              Leiharbeiter für KW {selectedWeekIndex !== null ? weeks[selectedWeekIndex].weekNumber : ''}
            </DialogTitle>
          </DialogHeader>
          <div className="py-4 space-y-2 max-h-96 overflow-y-auto">
            {tempWorkers.length === 0 ? (
              <p className="text-center text-gray-500 py-4">
                Keine Leiharbeiter verfügbar
              </p>
            ) : (
              tempWorkers.map(worker => (
                <div
      ...
      <Dialog open={workerDialogOpen} onOpenChange={setWorkerDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>
              {editingWorker ? 'Leiharbeiter bearbeiten' : 'Neuer Leiharbeiter'}
            </DialogTitle>
          </DialogHeader>
