  ...
  const handleAction = async () => {
    ...
    try {
      ...
      if (newStatus === 'genehmigt') {
        // The employee_id should already be set in the LeaveRequest
      ...
      if (requester && requester.email) {
        ...
        try {
          await base44.integrations.Core.SendEmail({
            ...
            body: `
Hallo ${selectedRequest.employee_name || selectedRequest.created_by || ''},

Ihre Krankmeldung wurde ${statusText}.
...
Ihr Leniger Team
  ...
  const getActionTitle = () => {
    if (actionType === 'approve') return 'Krankmeldung anerkennen';
    if (actionType === 'reject') return 'Krankmeldung ablehnen';
    return 'Krankmeldung in Prüfung setzen';
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-500 mt-1">Verwalten Sie Krankmeldungen</p>
      </div>
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            <Select value={statusFilter} onValueChange={setStatusFilter}>
              ...
              <SelectContent>
                ...
                <SelectItem value="genehmigt">Anerkannt</SelectItem>
          ...
          {filteredRequests.filter(r => r.status === 'eingereicht').length === 0 ? (
            <Card className="border-0 shadow-sm dark:bg-gray-800">
              <CardContent className="py-12 text-center">
                ...
                <p className="text-gray-500 dark:text-gray-400">Keine ausstehenden Krankmeldungen</p>
              </CardContent>
            </Card>
          ...
          {filteredRequests.filter(r => r.status === 'in_pruefung').length === 0 ? (
            <Card className="border-0 shadow-sm dark:bg-gray-800">
              <CardContent className="py-12 text-center">
                ...
                <p className="text-gray-500 dark:text-gray-400">Keine Krankmeldungen in Prüfung</p>
              </CardContent>
            </Card>
          ...
          {filteredRequests.filter(r => ['genehmigt', 'abgelehnt'].includes(r.status)).length === 0 ? (
            <Card className="border-0 shadow-sm dark:bg-gray-800">
              <CardContent className="py-12 text-center">
                ...
                <p className="text-gray-500 dark:text-gray-400">Keine abgeschlossenen Krankmeldungen</p>
              </CardContent>
            </Card>
            ...
            <div className="space-y-4 py-4">
              <div className="bg-gray-50 rounded-lg p-4 space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium">{selectedRequest.employee_name || selectedRequest.created_by || 'Unbekannt'}</span>
                </div>
                <div className="flex items-center gap-2">
...
function RequestCard({ request, onAction, showActions, users = [] }) {
  ...
  const getStatusBadge = (status) => {
    const config = {
      ...
      genehmigt: { icon: CheckCircle, class: 'bg-green-100 text-green-700 border-green-200', label: 'Anerkannt' },
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1 flex-wrap">
                <h3 className="font-semibold text-gray-900 dark:text-white">
                  {days} Tag{days !== 1 ? 'e' : ''} Krankmeldung
                </h3>
                {getStatusBadge(request.status)}
              </div>
              <p className="text-sm text-gray-600 dark:text-gray-400 mb-1">
                <User className="w-3 h-3 inline mr-1" />
                {request.employee_name || request.created_by || 'Unbekannt'}
              </p>
